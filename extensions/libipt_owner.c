      ...
      struct ipt_entry_match **match)
{
	...
	case '1':
		...
		if ((pwd = getpwnam(optarg)))
			...
			if (*end != '\0' || end == optarg)
				xtables_error(PARAMETER_PROBLEM, "Bad OWNER UID value `%s'", optarg);
		}
		if (invert)
	...
	case '2':
		...
		if ((grp = getgrnam(optarg)))
			...
			if (*end != '\0' || end == optarg)
				xtables_error(PARAMETER_PROBLEM, "Bad OWNER GID value `%s'", optarg);
		}
		if (invert)
	...
	case '3':
		...
		if (*end != '\0' || end == optarg)
			xtables_error(PARAMETER_PROBLEM, "Bad OWNER PID value `%s'", optarg);
		if (invert)
	...
	case '4':
		...
		if (*end != '\0' || end == optarg)
			xtables_error(PARAMETER_PROBLEM, "Bad OWNER SID value `%s'", optarg);
		if (invert)
	...
	case '5':
		...
		if(strlen(optarg) > sizeof(ownerinfo->comm))
			xtables_error(PARAMETER_PROBLEM, "OWNER CMD `%s' too long, max %u characters", optarg, (unsigned int)sizeof(ownerinfo->comm));
